...
class Embedding(nn.Module):
    def __init__(self,d_in,d_out):
        ...
        self.convs=nn.ModuleList([
            nn.Conv1d(d_in,d_out//4,i*2+7,stride=4,padding=i+3,bias=False) for i in range(4)
        ])
...
class Projector(nn.Module):
    def __init__(self,heads,dim):
        ...
        self.q_k_v=nn.ModuleList([
            nn.Sequential(nn.Conv2d(dim,dim,3,stride=1 if i==0 else 2,padding=1,groups=dim,bias=False),
            nn.BatchNorm2d(dim),
            nn.Conv2d(dim,dim,1,1,0),nn.GELU() ) for i in range(3)
        ])
...
class FFN(nn.Module):
    def __init__(self,dim,ratio=4):
        super().__init__()
        self.MLP=nn.Sequential(nn.Linear(dim,dim//ratio),nn.GELU(),
...
class CLFormer(nn.Module):
    def __init__(self,params):
        ...
        self.output_layer=nn.Linear(32,d_out)
        self.zero_last_layer_weight()
    def zero_last_layer_weight(self):
        self.output_layer.weight.data=torch.zeros_like(self.output_layer.weight)
        self.output_layer.bias.datas=torch.zeros_like(self.output_layer.bias)
